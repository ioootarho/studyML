...
**2, モデル配布パターン**  
- 学習したモデルを配布する（exeファイル, VBAツール, コンテナ, etc）
...
作成したLightGBMのモデルをデプロイする。  
このハンズオンでは図のように推論用のサーバを立てて、そこに対してAPIリクエストを実行するシンプルな構成で行う。  

![figure DeployArchitect](./figures/DeployArchitect.svg)  

なお、このハンズオンはGoogle Colaboratory ではなくDockerコンテナで実行することを想定している。  

**進め方**
- [モデルデプロイのノートブック](https://github.com/suchu3/studyML/blob/main/handson/src/ModelDeploy.ipynb)をDockerコンテナのJupyter-Labで開く
- モデルデプロイのノートブックのファイルパスは以下の通り
```
studyML/handson/src/ModelDeploy.ipynb
```
- コメントに沿って**オリジナル形式とONNX形式の推論速度比較**のコードまで実行する
- GitHubリポジトリの[推論サーバ構築手順](https://github.com/suchu3/studyML/tree/main/handson/deploy_handson)のコンテナ起動手順0~3に沿って推論サーバのコンテナを立ち上げる
- 推論サーバ構築手順のリクエスト実行手順に沿ってローカルホストからリクエストを投げて、推論結果が返ってくることを確認する
- モデルデプロイのノートブックに戻る
- **web APIとしてデプロイしたモデルで推論実行**のコードを実行して、推論結果が返ってくることを確認する
- 推論サーバ構築手順のコンテナ停止手順に沿って推論サーバのコンテナを停止させる

## MLOpsとは

機械学習モデルを実戦投入するためには、単に機械学習モデルを作るだけでなく、ソフトウェアエンジニアやサーバサイドエンジニアの知識・技術が必要。  
