...
\end{align}
\\]
- ただし、誤差項\\(\varepsilon\\)は平均$0$なので消える
...
\\[
\begin{align}
J(\beta_0, \beta_1) &= \frac{1}{n}\sum_{i=1}^{n}(crime-\hat{crime_i})^2 \cr
&= \frac{1}{n}\sum_{i=1}^{n} (crime-\beta_0 - \beta_1unemp_i)^2
\end{align}
\\]
...
特徴量行列$X$ ターゲットベクトル$y$ パラメータベクトル$\beta$ に対して
\\[
\beta = (X^TX)^{-1}X^Ty
\\]
